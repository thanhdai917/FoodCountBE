...
<?php 
    ...
    class helpers{
        ...
        public static function encryptCookie( $value ) {
            $key = 'qwertzuiopasdfghjklyxcvbnmQWERTZUIOPASDFGHJKLYXCVBNM0123456789!$/()*';
      ...
      public function megaMenu($categories, $parent_id = 0){
        $result = [];
        foreach($categories as $item){
          if($item['parent_id']==$parent_id){
            unset($categories[$item['id']]);
            $child = self::megaMenu($categories,$item['id']);
            array_push($result, new menuMain($item['id'],$item['name'],$item['slug'],$item['parent_id'],$child));
          }
        }
        return $result;
      }
    ...
    class menuMain{
      ...
      public $parent_id;
      public $child_menu;
      public function __construct($id,$name,$slug,$parent_id,$child_menu){
        $this->id = $id;
        $this->name = $name;
        $this->slug = $slug;
